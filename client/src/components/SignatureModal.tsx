...
interface SignatureMeta { id: string; title: string; width: number; height: number; updatedAt: string; }
...
export function SignatureModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const saveDrawn = async () => {
    ...
    if (!pad || pad.isEmpty()) { toast('error', 'Draw your signature first'); return; }
    setBusy(true);
    try {
  ...
  const saveUploaded = async () => {
    if (!file) { toast('error', 'Choose a photo of your signature first'); return; }
    setBusy(true);
    try {
  ...
  return (
    <Modal open={open} onClose={onClose} title="My Digital Signature">
      <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
        ...
        Draw it below, or photograph a paper signature — the paper background is removed automatically.
      </p>
      ...
      <div className="mb-3 flex gap-1 rounded-lg bg-slate-100 p-1 dark:bg-slate-800">
        {(['draw', 'upload'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'draw'
              ? <span className="inline-flex items-center gap-1.5"><Icon name="pen" size={14} /> Draw</span>
              : <span className="inline-flex items-center gap-1.5"><Icon name="camera" size={14} /> Photo / Scan</span>}
          </button>
        ))}
      </div>
      ...
      {tab === 'draw' ? (
        <div>
          ...
          <div className="mt-3 flex items-center justify-between">
            <button className="btn-ghost px-3 py-1.5 text-sm" onClick={() => padRef.current?.clear()}>Clear pad</button>
            <button className="btn-primary" onClick={() => void saveDrawn()} disabled={busy}>
              {busy ? 'Saving…' : 'Save Signature'}
            </button>
          </div>
          <p className="mt-2 text-xs text-slate-400">Tip: sign slowly with your finger or stylus for the smoothest result.</p>
        </div>
      ) : (
        <div>
          ...
          <input id="sig-upload" type="file" accept="image/*" capture="environment" className="hidden"
            onChange={(e) => pickFile(e.target.files?.[0] ?? null)} />
          <div className="mt-3 flex items-center justify-between">
            <span className="text-xs text-slate-400">{file ? file.name : ''}</span>
            <button className="btn-primary" onClick={() => void saveUploaded()} disabled={busy || !file}>
              {busy ? 'Processing…' : 'Clean & Save'}
            </button>
          </div>
          <p className="mt-2 text-xs text-slate-400">
            For photos: sign with a dark pen on plain white paper, photograph in good light, close-up. We handle rotation,
            background removal and cropping for you.
          </p>
        </div>
      )}
    </Modal>
