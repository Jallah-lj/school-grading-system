...
const NAV: { to: string; label: string; iconPath: string; roles: Role[] }[] = [
  { to: '/', label: 'Dashboard', iconPath: 'M3 12l9-9 9 9M5 10v10a1 1 0 001 1h4v-6h4v6h4a1 1 0 001-1V10', roles: ['ADMIN', 'TEACHER', 'STUDENT', 'PARENT'] },
  { to: '/grade-entry', label: 'Grade Entry', iconPath: 'M11 4H4v16h16v-7M18.5 2.5a2.1 2.1 0 013 3L12 15l-4 1 1-4 9.5-9.5z', roles: ['TEACHER', 'ADMIN'] },
  ...
  { to: '/analytics', label: 'Analytics', iconPath: 'M18 20V10M12 20V4M6 20v-6', roles: ['ADMIN', 'TEACHER'] },
  { to: '/audit-logs', label: 'Audit Logs', iconPath: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z', roles: ['ADMIN'] },
  { to: '/admin', label: 'Administration', iconPath: 'M12 15a3 3 0 100-6 3 3 0 000 6zM19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 11-2.83 2.83l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 11-4 0v-.09a1.65 1.65 0 00-1-1.51 1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 11-2.83-2.83l.06-.06a1.65 1.65 0 00.33-1.82 1.65 1.65 0 00-1.51-1H3a2 2 0 110-4h.09a1.65 1.65 0 001.51-1 1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 112.83-2.83l.06.06a1.65 1.65 0 001.82.33h.01a1.65 1.65 0 001-1.51V3a2 2 0 114 0v.09a1.65 1.65 0 001 1.51h.01a1.65 1.65 0 001.82-.33l.06-.06a2 2 0 112.83 2.83l-.06.06a1.65 1.65 0 00-.33 1.82v.01a1.65 1.65 0 001.51 1H21a2 2 0 110 4h-.09a1.65 1.65 0 00-1.51 1z', roles: ['ADMIN'] },
];
...
function ThemeToggle() {
  ...
  return (
    <button onClick={toggle} className="btn-ghost px-2.5 py-2" aria-label="Toggle theme" title="Toggle dark / light mode">
...
function NotificationBell() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button className="btn-ghost relative px-2.5 py-2" onClick={() => { setOpen((o) => !o); if (!open) void load(); }} aria-label="Notifications">
      ...
      {open && (
        <div className="card absolute right-0 z-40 mt-2 w-80 overflow-hidden p-0">
          ...
          <div className="max-h-80 overflow-y-auto">
            ...
            {items.map((n) => (
              <div key={n.id}
                className={cx('group relative border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/60', !n.isRead && 'bg-indigo-50/50 dark:bg-indigo-500/5')}>
                <button onClick={() => void markRead(n)} className="block w-full px-4 py-3 pr-9 text-left">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-[11px] text-slate-400">
                    {fmtDate(n.createdAt)}
                    {n.link && <span className="inline-flex items-center gap-1 font-medium text-indigo-500">Open <Icon name="arrow-right" size={12} /></span>}
                  </div>
                </button>
                <button
...
export function AppLayout() {
  ...
  useEffect(() => {
    ...
    const loadCount = () => api
      ...
      .then((r) => { if (!cancelled) setPendingApprovals(r.data.data.length); })
      .catch(() => undefined);
    loadCount();
    ...
    <div className="flex h-full flex-col">
        ...
        {links.map((n) => (
          <NavLink key={n.to} to={n.to} end={n.to === '/'} onClick={() => setMobileOpen(false)}
            ...
            {n.to === '/approvals' && pendingApprovals > 0 && (
              <span className="ml-auto flex h-5 min-w-5 items-center justify-center rounded-full bg-rose-500 px-1.5 text-[11px] font-bold text-white" title={`${pendingApprovals} submission(s) awaiting approval`}>
      ...
      <div className="border-t border-slate-200 p-4 dark:border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <button className="btn-ghost px-2 py-1.5" title="Sign out" onClick={() => { void logout().then(() => navigate('/login')); }}>
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex min-h-screen flex-1 flex-col lg:pl-64">
        <header className="sticky top-0 z-20 flex items-center gap-2 border-b border-slate-200 bg-white/80 px-4 py-3 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80">
          <button className="btn-ghost px-2 py-1.5 lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open menu">
            <Icon name="menu" size={20} />
          </button>
          <div className="flex-1" />
          {hasRole('TEACHER', 'ADMIN') && (
            <button className="btn-ghost px-2.5 py-2" title="My digital signature" aria-label="My digital signature"
