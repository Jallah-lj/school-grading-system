...
function AdminReportCards() {
  ...
  const { data, loading, refetch } = useQuery(
    () =>
      ready
        ? api
            .get<{ data: ReportCardListItem[] }>(
              `/report-cards?classId=${classId}&semesterId=${semesterId}`,
            )
            .then((r) => r.data)
  ...
  const generate = async () => {
    setBusy(true);
    try {
      const { data: res } = await api.post<{ generated: number; skipped: number }>(
        '/report-cards/generate',
        { classId, semesterId },
      );
      toast(
        'success',
        `Generated ${res.generated} report cards${res.skipped ? ` (${res.skipped} skipped — approve grades first)` : ''}`,
      );
      void refetch();
    } catch (err) {
  ...
  const publishAll = async () => {
    setBusy(true);
    try {
      const { data: res } = await api.post<{ published: number; notified: number }>(
        '/report-cards/publish-all',
        { classId, semesterId },
      );
  ...
  const downloadClassCsv = async () => {
    try {
      const res = await api.get(
        `/reports/class-report.csv?classId=${classId}&semesterId=${semesterId}`,
        { responseType: 'blob' },
      );
  ...
  return (
    <div className="space-y-4">
      ...
      {!ready ? (
        <EmptyState title="Select a term and class" />
      ) : loading ? (
        <div className="card">
          <TableSkeleton />
        </div>
      ...
      <Modal
        ...
        title={`Remarks — ${editing?.student.name ?? ''}`}
      >
        <div className="space-y-4">
          <div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="btn-primary" onClick={() => void saveRemarks()} disabled={busy}>
              Save Remarks
            </button>
          </div>
        </div>
  ...
  const { data, loading } = useQuery(
    () =>
      api
        ...
        }>(`/report-cards/mine${studentId ? `?studentId=${studentId}` : ''}`)
        .then((r) => r.data),
    [studentId],
  );
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          hint="You'll be notified as soon as a report card is published."
        />
      ) : (
          ...
          {data.data.map((c) => (
            <div key={c.id} className="card p-5">
              ...
              <div className="mt-1 font-mono text-xs text-slate-400">
                Verify code: {c.verificationCode}
              </div>
