...
export default function AuditLogs() {
  ...
  useEffect(() => { setPage(1); }, [debounced, action, entity, from, to]);

  const { data: meta } = useQuery(
    () => api.get<{ actions: string[]; entities: string[] }>('/admin/audit-logs/meta').then((r) => r.data),
    [],
  );
  ...
  const clearFilters = () => { setSearch(''); setAction(''); setEntity(''); setFrom(''); setTo(''); };
  const hasFilters = search || action || entity || from || to;

  return (
    <div>
      <PageHeader title="Audit Logs" subtitle="Every sensitive action in the system is recorded here — who did what, when, and from where." />

      <div className="card mb-4 flex flex-wrap items-center gap-3 p-4">
        <input className="input max-w-xs" placeholder="Search action, entity, user…"
          value={search} onChange={(e) => setSearch(e.target.value)} />
        <select className="input max-w-[220px]" value={action} onChange={(e) => setAction(e.target.value)}>
        ...
        {hasFilters && (
          <button className="btn-ghost px-3 py-1.5 text-xs" onClick={clearFilters}><Icon name="x" size={12} /> Clear filters</button>
        )}
        {data && <span className="ml-auto text-sm text-slate-400">{data.total} event{data.total === 1 ? '' : 's'}</span>}
      </div>

      <div className="card overflow-hidden">
        {loading ? <TableSkeleton rows={10} cols={6} /> :
          error ? <EmptyState title="Failed to load" hint={error} /> :
          !data || data.data.length === 0 ? (
            <EmptyState title="No audit events found" hint={hasFilters ? 'Try widening your filters.' : 'Events will appear here as the system is used.'} />
          ) : (
                ...
                {data.data.map((log) => (
                  <Fragment key={log.id}>
                    <tr className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/40">
                      <td className="td whitespace-nowrap text-xs text-slate-500 dark:text-slate-400">{fmtDateTime(log.createdAt)}</td>
                      <td className="td">
                        {log.user ? (
                          <>
                            ...
                            <div className="flex items-center gap-1.5 text-xs text-slate-400">
                              ...
                              <Badge className={roleTone[log.user.role] ?? ''}>{log.user.role}</Badge>
                            </div>
                          </>
                        ) : <span className="text-slate-400">System</span>}
                      </td>
                      <td className="td">
                        <Badge className={cx('font-mono text-[11px]', actionTone(log.action))}>{log.action}</Badge>
                      </td>
                      <td className="td">
                        <div className="text-sm">{log.entity}</div>
                        {log.entityId && <div className="max-w-[140px] truncate font-mono text-xs text-slate-400" title={log.entityId}>{log.entityId}</div>}
                      </td>
                      ...
                      <td className="td text-right">
                        {log.metadata != null && (
                          <button className="btn-ghost px-2 py-1 text-xs"
                            ...
                            {expanded === log.id ? <><Icon name="chevron-down" size={12} /> Hide</> : <><Icon name="chevron-right" size={12} /> View</>}
                          </button>
                        )}
                      </td>
                    </tr>
