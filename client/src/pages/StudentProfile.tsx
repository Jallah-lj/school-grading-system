...
import { cx, downloadBlob, fmtDate, gradeBadgeClass, initials, ordinal, statusBadgeClass } from '../lib/utils';
...
interface StudentDetail {
  ...
  createdAt: string;
  user: { id: string; name: string; email: string; isActive: boolean; lastLoginAt?: string | null; phone?: string | null };
...
export default function StudentProfile({ profileId, self = false }: StudentProfileProps = {}) {
  ...
  const downloadTranscript = async () => {
    if (!student) return;
    try {
      const res = await api.get(`/report-cards/transcript/${student.id}/pdf`, { responseType: 'blob' });
  ...
  return (
    <div>
        ...
        <div className="px-6 pb-6">
          <div className="-mt-10 flex flex-wrap items-end gap-4">
            <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-white text-2xl font-extrabold text-indigo-700 shadow-lg ring-4 ring-white dark:bg-slate-800 dark:text-indigo-300 dark:ring-slate-900">
              {student.photoUrl
                ? <img src={student.photoUrl} alt="" className="h-full w-full rounded-2xl object-cover" />
                : initials(student.user.name)}
            </div>
            <div className="min-w-0 flex-1 pt-10 sm:pt-2">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{student.user.name}</h1>
                <Badge className={student.user.isActive
              ...
              <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-slate-500 dark:text-slate-400">
                <span className="font-mono text-xs font-semibold text-indigo-600 dark:text-indigo-400">{student.admissionNumber}</span>
                <span>·</span>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'Term GPA', value: latestGpa ? latestGpa.gpa.toFixed(2) : '—', hint: latestGpa ? `${latestGpa.semester.name}` : 'No results yet', tone: 'indigo' as const, icon: 'award' as const },
          { label: 'Average', value: latestGpa ? `${latestGpa.average.toFixed(1)}%` : '—', hint: latestGpa ? `${latestGpa.totalCredits} credits` : undefined, tone: 'sky' as const, icon: 'bar-chart' as const },
          { label: 'Class position', value: latestGpa?.position ? ordinal(latestGpa.position) : '—', hint: latestGpa?.classSize ? `of ${latestGpa.classSize}` : undefined, tone: 'emerald' as const, icon: 'trending-up' as const },
          { label: 'Report cards', value: String(student.reportCards.length), hint: student.reportCards.filter((c) => c.status === 'PUBLISHED').length + ' published', tone: 'amber' as const, icon: 'file-text' as const },
        ].map((s) => (
          <div key={s.label} className="card flex items-center gap-4 p-4">
            <div className={cx(
              ...
              s.tone === 'amber' && 'bg-amber-100 text-amber-600 dark:bg-amber-500/15 dark:text-amber-400',
            )}>
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="card p-5">
            ...
            <dl className="space-y-3 text-sm">
              {[
                ...
                ['Class', student.classRoom ? `${student.classRoom.name} ${student.classRoom.stream}` : '— Unassigned —'],
          ...
          {student.parent && (
            <div className="card p-5">
              ...
              <div className="font-medium text-slate-900 dark:text-white">{student.parent.user.name}</div>
            ...
            <div className="card p-5">
              ...
              <ul className="space-y-2">
                {student.enrollments.map((e) => (
                  <li key={e.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm dark:bg-slate-800/60">
                    <span className="font-medium">{e.classRoom.name} {e.classRoom.stream}</span>
                    <span className="text-xs text-slate-400">{e.semester.name} · {e.semester.academicYear.name}</span>
                  </li>
                ))}
              </ul>
            </div>
              ...
              <h2 className="font-semibold text-slate-900 dark:text-white">
                ...
                {results?.semester && (
                  <span className="ml-2 text-sm font-normal text-slate-400">
                    ...
                    {results.semester.academicYear ? ` · ${results.semester.academicYear.name}` : ''}
                  </span>
                )}
              </h2>
            </div>
            {resultsLoading ? <TableSkeleton rows={4} cols={5} /> : !results?.results.length ? (
              <EmptyState title="No results yet" hint="Results appear after grades are approved for this term." icon="book" />
            ) : (
                  ...
                  <tbody>
                    {student.gpaRecords.map((g) => (
                      <tr key={g.id} className="border-t border-slate-100 dark:border-slate-800">
                        ...
                        <td className="td text-right font-bold text-indigo-600 dark:text-indigo-400">{g.gpa.toFixed(2)}</td>
                        <td className="td text-right">{g.average.toFixed(1)}%</td>
                        <td className="td text-center">{g.position ? `${ordinal(g.position)} / ${g.classSize ?? '—'}` : '—'}</td>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {student.reportCards.map((c) => (
                  <div key={c.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
                    <div>
                      <div className="font-medium">{c.semester.name} · {c.semester.academicYear.name}</div>
                      <div className="text-xs text-slate-400">
                        {c.publishedAt ? `Published ${fmtDate(c.publishedAt)}` : 'Not published'} · {c.verificationCode}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge className={statusBadgeClass(c.status)}>{c.status}</Badge>
                      <Link to={`/verify/${c.verificationCode}`} target="_blank" className="btn-ghost px-2 py-1 text-xs">
                        View
                      </Link>
                    </div>
                  </div>
