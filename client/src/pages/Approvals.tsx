...
export default function Approvals() {
  ...
  const sendBack = async () => {
    ...
    try {
      ...
      await api.post('/grades/unlock', { classId, subjectId, semesterId, to: 'DRAFT', note: note.trim() || undefined });
  ...
  return (
    <div>
        ...
        {loading ? <TableSkeleton rows={4} cols={7} /> :
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[860px]">
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={`${r.classId}-${r.subjectId}-${r.semesterId}`}
                    ...
                    <td className="td font-medium">{r.className} {r.stream}</td>
                    <td className="td">
                    ...
                    <td className="td">
                      <div className="flex justify-end gap-2">
                        ...
                        <button className="btn-ghost px-3 py-1.5 text-xs text-amber-600 dark:text-amber-400"
                          onClick={() => { setReturning(r); setNote(''); }}>
                          <Icon name="corner-up-left" size={13} /> Return
                        </button>
                        <button className="btn-primary px-3 py-1.5 text-xs" onClick={() => setApproving(r)}>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => void approve()} onCancel={() => setApproving(null)} />

      <Modal open={!!returning} onClose={() => setReturning(null)} title="Return for correction">
        <p className="mb-4 text-sm text-slate-600 dark:text-slate-300">
          Send <strong>{returning?.className} {returning?.stream} — {returning?.subjectName}</strong> back to
          {returning?.teachers.length ? ` ${returning.teachers.join(', ')}` : ' the teacher'} as a draft.
          They will be notified and can edit and resubmit.
        </p>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button className="btn-danger" onClick={() => void sendBack()} disabled={busy}>
            {busy ? 'Returning…' : <><Icon name="corner-up-left" size={14} /> Return to teacher</>}
          </button>
        </div>
      </Modal>
    </div>
