...
const teacherSchema = z.object({
  ...
  password: z.string().min(8, 'Min 8 characters').regex(/[A-Za-z]/).regex(/[0-9]/),
...
export default function Teachers() {
  ...
  const addAssignment = async () => {
    ...
    try {
      await api.post(`/teachers/${assigning.id}/assignments`, { subjectId: assignSubject, classId: assignClass });
      toast('success', 'Assignment added');
      setAssignSubject(''); setAssignClass('');
      void refetch();
    } catch (err) { toast('error', apiError(err)); }
  };
  ...
  return (
    <div>
      <PageHeader title="Teachers" subtitle="Manage teacher accounts and their subject/class assignments."
        actions={<button className="btn-primary" onClick={() => setModalOpen(true)}><Icon name="user-plus" size={15} /> Add Teacher</button>} />

      <div className="card mb-4 p-4">
        <input className="input max-w-xs" placeholder="Search name or staff number…"
          ...
          {data.data.map((t) => (
            <div key={t.id} className="card p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {t.assignments.length === 0 && <span className="text-xs text-slate-400">No assignments yet</span>}
                {t.assignments.map((a) => (
                  <Badge key={a.id ?? `${a.subject.id}-${a.classRoom.id}`} className="bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add Teacher">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2"><label className="label">Full name</label><input className="input" {...register('name')} />{errors.name && <p className="mt-1 text-xs text-rose-500">{errors.name.message}</p>}</div>
            <div><label className="label">Email</label><input className="input" type="email" {...register('email')} />{errors.email && <p className="mt-1 text-xs text-rose-500">{errors.email.message}</p>}</div>
            ...
            <div><label className="label">Qualification (optional)</label><input className="input" {...register('qualification')} placeholder="B.Ed, M.Ed…" /></div>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" className="btn-secondary" onClick={() => setModalOpen(false)}>Cancel</button>
            <button type="submit" className="btn-primary" disabled={isSubmitting}>{isSubmitting ? 'Creating…' : 'Create Teacher'}</button>
          </div>
        </form>
      </Modal>

      <Modal open={!!assigning} onClose={() => setAssigning(null)} title={`Assign — ${assigning?.user.name ?? ''}`}>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <button className="btn-secondary" onClick={() => setAssigning(null)}>Done</button>
            <button className="btn-primary" onClick={() => void addAssignment()} disabled={!assignSubject || !assignClass}>Add Assignment</button>
          </div>
        </div>
      </Modal>
