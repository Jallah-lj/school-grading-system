...
export default function MyGrades() {
  ...
  const { data, loading } = useQuery(
    () =>
      studentId
        ? api
            .get<StudentResultsResponse>(
              `/students/${studentId}/results${semesterId ? `?semesterId=${semesterId}` : ''}`,
            )
            .then((r) => r.data)
  ...
  const { data: trend } = useQuery(
    () =>
      studentId
        ? api
            .get<{ points: TrendPoint[] }>(`/analytics/gpa-trends?studentId=${studentId}`)
            .then((r) => r.data)
        : Promise.resolve(undefined),
    [studentId],
  );
  ...
  const downloadTranscript = async () => {
    try {
      const res = await api.get(`/report-cards/transcript/${studentId}/pdf`, {
        responseType: 'blob',
      });
  ...
  if (!studentId) {
    return (
      <EmptyState
        ...
        hint={
          isParent
            ? 'Select one of your children above.'
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <button className="btn-secondary" onClick={() => void downloadTranscript()}>
            <Icon name="download" size={15} /> Download Transcript
          </button>
        }
      />
      ...
      <div className="card mb-4 flex flex-wrap items-center gap-3 p-4">
        ...
        {data?.semester && (
          <span className="ml-auto text-sm text-slate-400">{data.semester.name}</span>
        )}
      </div>
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-4">
            {[
              ...
              },
              {
                ...
                sub: data.gpa?.classSize ? `of ${data.gpa.classSize}` : 'class',
              },
              {
                label: 'Credits',
                value: data.gpa ? String(data.gpa.totalCredits) : '—',
                sub: 'credit units',
              },
            ].map((s) => (
          ...
          <div className="card overflow-hidden">
            ...
            {data.results.length === 0 ? (
              <EmptyState title="No published results for this term" />
            ) : (
