...
import type { ClassRoom, ImportResult, Paged, StudentRow } from '../lib/types';
...
function ImportStudentsModal({ open, onClose, onDone }: { open: boolean; onClose: () => void; onDone: () => void }) {
  ...
  const runImport = async () => {
    ...
    try {
      ...
      if (data.failed > 0) toast('error', `${data.failed} row${data.failed === 1 ? '' : 's'} failed — review below`);
      ...
      {!result ? (
        <div className="space-y-4">
          <ol className="list-inside list-decimal space-y-1.5 text-sm text-slate-600 dark:text-slate-300">
            <li>Download the template — it lists the required columns and the exact class names on its <em>Classes</em> sheet.</li>
            <li>Fill one student per row. Admission numbers are assigned automatically.</li>
            <li>Leave <code className="rounded bg-slate-100 px-1 font-mono text-xs dark:bg-slate-800">password</code> blank to auto-generate it — you'll get the full credentials list afterwards.</li>
            <li>Upload the filled file. Bad rows are skipped and reported; the rest are imported.</li>
          </ol>
          ...
          <p className="text-xs text-slate-400">Formats: .xlsx or .csv · max 5 MB · 500 rows per batch.</p>
          <div className="flex justify-end gap-2">
            <button className="btn-secondary" onClick={onClose}>Cancel</button>
            <button className="btn-primary" disabled={!file || busy} onClick={() => void runImport()}>
              {busy ? 'Importing…' : <><Icon name="upload" size={15} /> Import students</>}
            </button>
          </div>
        </div>
      ) : (
        <div className="space-y-5">
          <div className={cx('flex items-center gap-3 rounded-xl p-3 text-sm',
            ...
            <span><strong>{result.created}</strong> imported · <strong>{result.failed}</strong> skipped</span>
          </div>
            ...
            <table className="w-full">
              <thead className="border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  <th className="th">Status</th>{hasRole('ADMIN') && <th className="th text-right">Actions</th>}
                </tr>
              </thead>
              <tbody>
                {data.data.map((s) => (
