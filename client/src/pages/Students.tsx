      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            {editing ? (
              <input
                className="input bg-slate-50 font-mono dark:bg-slate-800/60"
            ...
            <p className="mt-1 text-xs text-slate-400">
              System-generated — unique and conflict-free.
            </p>
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            <label className="label">
              Parent email (optional — links an existing parent account)
            </label>
            <input
              className="input"
              type="email"
            ...
            />
            {errors.parentEmail && (
              <p className="mt-1 text-xs text-rose-500">{errors.parentEmail.message}</p>
            )}
          </div>
        </div>
...
function ImportStudentsModal({
  ...
  const runImport = async () => {
    ...
    try {
      ...
      if (data.created > 0) {
        toast(
          'success',
          `Imported ${data.created} student${data.created === 1 ? '' : 's'} from ${data.file ?? 'file'}`,
        );
        onDone();
      }
      if (data.failed > 0)
        toast('error', `${data.failed} row${data.failed === 1 ? '' : 's'} skipped — review below`);
  ...
  const credCsv = (rows: ImportResult['credentials']) =>
    [
      'name,email,password',
      ...rows.map((c) => `${JSON.stringify(c.name)},${c.email},${JSON.stringify(c.password)}`),
    ].join('\n');
      ...
      {step === 'upload' && (
        <div className="space-y-4">
          <ol className="list-inside list-decimal space-y-1.5 text-sm text-slate-600 dark:text-slate-300">
            <li>
              Download the template — it lists required columns and exact class names on the{' '}
              <em>Classes</em> sheet.
            </li>
            ...
            <li>
              ...
              blank to auto-generate — you&apos;ll get credentials after import.
            </li>
            <li>
              Upload the file and <strong>preview</strong> first. Fix any problems, then confirm the
              import.
            </li>
          </ol>
          <div className="flex flex-wrap items-center gap-3 rounded-xl border border-dashed border-slate-300 p-4 dark:border-slate-700">
            ...
            <button className="btn-secondary" onClick={() => void downloadTemplate()}>
              <Icon name="download" size={15} /> Download template (.xlsx)
            </button>
          ...
          <p className="text-xs text-slate-400">
            Formats: .xlsx or .csv · max 5 MB · 500 rows per batch. Nothing is saved until you
            confirm after the preview.
          </p>
          <div className="flex justify-end gap-2">
            ...
            >
              {busy ? (
                ...
                <>
                  <Icon name="eye" size={15} /> Preview import
                </>
              )}
            </button>
          </div>
        ...
        <div className="space-y-5">
          ...
          {preview.errors.length > 0 && (
            <div>
              ...
              <p className="mb-2 text-xs text-slate-400">
                These rows will be skipped. Fix them in your spreadsheet and re-upload, or continue
                to import only the ready rows.
              </p>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {preview.errors.map((e) => (
                      <tr
                        key={`${e.row}-${e.email}`}
                        className="border-t border-rose-100 dark:border-rose-900/40"
                      >
              ...
              <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-emerald-700 dark:text-emerald-400">
                ...
                {preview.previewTotal > preview.preview.length && (
                  <span className="font-normal text-slate-400">
                    {' '}
                    (showing first {preview.preview.length} of {preview.previewTotal})
                  </span>
                )}
              </h3>
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <button className="btn-secondary" onClick={resetToUpload}>
              <Icon name="corner-up-left" size={14} /> Choose different file
            </button>
            <button
              className="btn-primary"
            ...
            >
              {busy ? (
                ...
                <>
                  <Icon name="upload" size={15} /> Confirm import ({preview.ready})
                </>
              )}
            </button>
          </div>
      ...
      {step === 'done' && result && (
        <div className="space-y-5">
          ...
          >
            ...
            <span>
              <strong>{result.created}</strong> imported · <strong>{result.failed}</strong> skipped
                ...
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-slate-50 dark:bg-slate-800">
                    <tr>
                      ...
                      <th className="th py-2">Reason</th>
                    </tr>
                  </thead>
                  <tbody>
                    {result.errors.map((e) => (
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <h3 className="flex items-center gap-1.5 text-sm font-semibold">
                  <Icon name="key" size={15} /> Login credentials (share with students)
                </h3>
                <button
                  className="btn-secondary px-3 py-1 text-xs"
                  onClick={() =>
                    downloadBlob(
                      new Blob([credCsv(result.credentials)], { type: 'text/csv' }),
...
export default function Students() {
  ...
  const { data, loading, error, refetch } = useQuery(
    () => api.get<Paged<StudentRow>>(`/students?${params}`).then((r) => r.data),
    [params],
  );
  ...
  return (
    <div>
      <PageHeader
        title="Students"
        subtitle="Register, search and manage student records."
        actions={
          hasRole('ADMIN') ? (
            <>
              <button className="btn-secondary" onClick={() => setImportOpen(true)}>
                <Icon name="upload" size={15} /> Import from Excel
              </button>
              <button
                className="btn-primary"
                onClick={() => {
              ...
              >
                <Icon name="user-plus" size={15} /> Register Student
              </button>
            </>
          ) : undefined
        }
            ...
            <table className="w-full">
              <thead className="border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  >
                    Adm. No{sortArrow('admissionNumber')}
                  </th>
                  <th className="th cursor-pointer select-none" onClick={() => toggleSort('name')}>
              ...
              <tbody>
                {data.data.map((s) => (
                  ...
                  >
                    ...
                    <td className="td">
                      <Link to={`/students/${s.id}`} className="group flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <div className="font-medium text-indigo-700 group-hover:underline dark:text-indigo-300">
                            {s.user.name}
                          </div>
                    ...
                    <td className="td">
                      {s.classRoom ? (
                        `${s.classRoom.name} ${s.classRoom.stream}`
                      ) : (
                    ...
                    {hasRole('ADMIN') && (
                      <td className="td text-right">
                        <Link to={`/students/${s.id}`} className="btn-ghost px-2 py-1 text-xs">
                          View
                        </Link>
                        <button
      ...
      <PasswordConfirmDialog
        open={!!deleting}
        busy={busy}
        title="Delete student — security check"
