...
Resolved automatically onto report cards: homeroom teacher → _Class Teacher_ slot; publishing admin → _Principal_ slot. Exposed publicly only on **published** cards via `/report-cards/verify/:code` (`signatures.classTeacher/principal.dataUrl`).
...
| POST   | `/announcements/broadcast`                                                    | ADMIN           | Broadcast to `ALL`/`STUDENTS`/`PARENTS`/`TEACHERS`/`STUDENTS_AND_PARENTS`; in-app + optional email & SMS          |
| GET    | `/announcements?search=&page=&pageSize=`                                      | ADMIN           | Previously broadcast announcements                                                                                |
