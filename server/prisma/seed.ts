...
import { ensureEnrollments, recomputeGpas, recomputeSubjectResults } from '../src/services/results.service';
...
async function main() {
 ...
 prisma.schoolSetting.deleteMany(), prisma.idSequence.deleteMany(),
 prisma.reportCard.deleteMany(), prisma.gPARecord.deleteMany(),
 prisma.subjectResult.deleteMany(), prisma.gradeEntry.deleteMany(),
 prisma.enrollment.deleteMany(), prisma.teacherAssignment.deleteMany(),
 prisma.assessmentComponent.deleteMany(), prisma.gradeScaleBand.deleteMany(),
 prisma.gradeScale.deleteMany(), prisma.subject.deleteMany(),
 prisma.studentProfile.deleteMany(), prisma.teacherProfile.deleteMany(),
 prisma.parentProfile.deleteMany(), prisma.classRoom.deleteMany(),
 prisma.semester.deleteMany(), prisma.academicYear.deleteMany(),
 prisma.signature.deleteMany(), prisma.refreshToken.deleteMany(), prisma.user.deleteMany(),
 ]);
 ...
 hashPassword('Admin@123'), hashPassword('Teacher@123'),
 hashPassword('Student@123'), hashPassword('Parent@123'),
 ]);

 const admin = await prisma.user.create({
 data: { email: 'admin@school.rw', name: 'Aline Uwase', role: Role.ADMIN, passwordHash: adminHash },
 });
 const parentUser = await prisma.user.create({
 data: { email: 'parent@school.rw', name: 'Jean Bosco Nkurunziza', role: Role.PARENT, passwordHash: parentHash },
 });
 const parent = await prisma.parentProfile.create({ data: { userId: parentUser.id } });

 const teacherDefs = [
 { email: 'm.habimana@school.rw', name: 'Emmanuel Habimana', staffNumber: 'STF-001', qualification: 'B.Ed Mathematics' },
 { email: 'c.mukamana@school.rw', name: 'Claudine Mukamana', staffNumber: 'STF-002', qualification: 'B.Ed Sciences' },
 { email: 'p.nsengimana@school.rw', name: 'Patrick Nsengimana', staffNumber: 'STF-003', qualification: 'B.Ed Humanities' },
 ];
 ...
 data: {
 name: '2025–2026', startDate: new Date('2025-09-08'), endDate: new Date('2026-07-10'), isActive: true,
 semesters: {
 create: [
 { name: 'Term 1', number: 1, kind: 'TERM', startDate: new Date('2025-09-08'), endDate: new Date('2025-12-05') },
 { name: 'Term 2', number: 2, kind: 'TERM', startDate: new Date('2026-01-12'), endDate: new Date('2026-04-03') },
 { name: 'Term 3', number: 3, kind: 'TERM', startDate: new Date('2026-04-28'), endDate: new Date('2026-07-10'), isCurrent: true },
 ],
 },
 },
 ...
 console.log(` academic year ${year.name} with ${year.semesters.length} terms (current: ${term3.name})`);

 // ── Classes ────────────────────────────────────────────────────────────────
 const s1a = await prisma.classRoom.create({
 data: { name: 'Senior 1', level: 1, stream: 'A', academicYearId: year.id, homeroomTeacherId: teachers[0].id },
 });
 const s2a = await prisma.classRoom.create({
 data: { name: 'Senior 2', level: 2, stream: 'A', academicYearId: year.id, homeroomTeacherId: teachers[1].id },
 });
 ...
 data: {
 code: def.code, name: def.name, creditUnits: def.creditUnits, department: def.department,
 ...
 // ── Students ───────────────────────────────────────────────────────────────
 const studentNames = [
 ['Aline Ingabire', Gender.FEMALE], ['Kevin Mugisha', Gender.MALE], ['Diane Umutoni', Gender.FEMALE],
 ['Eric Niyonkuru', Gender.MALE], ['Grace Iradukunda', Gender.FEMALE], ['Samuel Hakizimana', Gender.MALE],
 ['Raissa Uwimbabazi', Gender.FEMALE], ['David Byiringiro', Gender.MALE], ['Sandrine Mutesi', Gender.FEMALE],
 ['Fabrice Ndayambaje', Gender.MALE], ['Olive Ishimwe', Gender.FEMALE], ['Jean Paul Twizeyimana', Gender.MALE],
 ];
 ...
 const components = await prisma.assessmentComponent.findMany({ where: { subjectId: subject.id } });
 ...
 data: {
 studentId: student.id, subjectId: subject.id, semesterId: term2.id,
 componentId: component.id, score, status: 'APPROVED',
 enteredById: teachers[subject.teacherIdx].id, approvedById: admin.id,
 submittedAt: new Date('2026-03-28'), approvedAt: new Date('2026-04-01'),
 },
 });
 }
 }
 ...
 await prisma.gradeEntry.updateMany({ where: { semesterId: term2.id }, data: { status: 'PUBLISHED', publishedAt: new Date('2026-04-02') } });
 await prisma.subjectResult.updateMany({ where: { semesterId: term2.id }, data: { isPublished: true } });
 await prisma.gPARecord.updateMany({ where: { semesterId: term2.id }, data: { isPublished: true } });

 // ── Term-2 report cards (published) ────────────────────────────────────────
 const { randomBytes } = await import('crypto');
 for (const student of students) {
 const gpa = await prisma.gPARecord.findUnique({ where: { studentId_semesterId: { studentId: student.id, semesterId: term2.id } } });
 if (!gpa) continue;
 await prisma.reportCard.create({
 data: {
 studentId: student.id, semesterId: term2.id, status: 'PUBLISHED',
 verificationCode: randomBytes(5).toString('hex').toUpperCase(),
 teacherRemarks: gpa.gpa >= 3 ? 'A very good performance. Keep it up!' : 'A fair performance — more effort is needed next term.',
 ...
 await prisma.gradeEntry.create({
 data: {
 studentId: student.id, subjectId: mat.id, semesterId: term3.id,
 componentId: component.id, score: Math.round((50 + rand() * 45) * 10) / 10,
 status: 'DRAFT', enteredById: teachers[0].id,
 },
 });
 }
 }
 ...
 create: { userId: s.userId, title: s.title, data: processed.png, width: processed.width, height: processed.height },
 update: { title: s.title, data: processed.png, width: processed.width, height: processed.height },
 });
 }
 ...
 where: { id: 'school' },
 create: { id: 'school', name: 'Excellence Model School', motto: 'Knowledge. Integrity. Excellence.', badgeData: badgePng, badgeMime: 'image/png' },
 update: {},
 });
 console.log(' school settings + demo badge installed');

 const [cStudents, cEntries, cResults] = await Promise.all([
 prisma.studentProfile.count(), prisma.gradeEntry.count(), prisma.subjectResult.count(),
 ]);

 console.log('----------------------------------------------------------------');
 console.log(' Seed complete');
 console.log(` Students: ${cStudents} · Grade entries: ${cEntries} · Computed results: ${cResults}`);
 console.log('');
