...
export interface SignatureSlot {
  ...
  name: string;
  png: Buffer | null;
  /** Printed above the line like a filled-in signature (used for the date). */ prefill?: string;
}
...
export function wrapText(
  ...
  while (i < words.length) {
    ...
    if (estimateWidth(next, size, font) <= maxW) {
      cur = next;
      i += 1;
      continue;
    }
  ...
  while (last.length > 1 && estimateWidth(`${last}…`, size, font) > maxW)
    ...
    row.forEach((cell, ci) => {
      ...
      if (ci === gradeCol) {
        ...
        b.emit({
          ...
          align: col.align ?? 'left',
          size: ci === 0 ? 8 : 9,
          font: isKey ? FONT.serifBold : FONT.serif,
          color: ci === row.length - 1 && !isKey ? COLOR.muted : COLOR.ink,
        });
      }
      cx += col.width;
    });
...
export function drawLegend(b: LayoutBuilder, bands: LegendBand[]): void {
  ...
  bands.forEach((band, i) => {
    ...
    b.emit({
      kind: 'text',
      text: `${bandRange(band)} · ${band.remark}`,
      x,
      y: y + 12.5,
      width: cellW,
      align: 'center',
...
function drawTermBlock(
  b: LayoutBuilder,
  columns: TableColumn[],
  gradeCol: number | undefined,
  term: { heading: string; meta: string; rows: string[][] },
): void {
  const headH = 14;
  const metaH = 12;
  ...
  switch (op.kind) {
    case 'rect':
      if (op.fill && op.stroke)
        doc
          ...
          .fillAndStroke(op.fill, op.stroke);
    ...
    case 'text':
      ...
      doc.text(op.text, op.x, op.y, {
        ...
        align: op.align,
        lineBreak: false,
        characterSpacing: op.charSpacing ?? 0,
        ellipsis: true,
      });
      break;
    case 'image':
