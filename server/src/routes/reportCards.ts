...
import {
  buildReportCardPdf, buildTranscriptPdf, loadReportCardDataFull,
...
const autoRemark = (gpa: number): string =>
  gpa >= 3.7 ? 'An outstanding performance. Keep up the excellent work.'
    : gpa >= 3.0 ? 'A very good performance. Aim even higher next term.'
    : gpa >= 2.5 ? 'A good performance with room to grow in a few subjects.'
    : gpa >= 2.0 ? 'A fair performance. More consistent effort is needed.'
...
reportCardsRouter.post('/generate', authenticate, authorize(Role.ADMIN), ah(async (req, res) => {
  const { classId, semesterId } = parseBody(z.object({ classId: z.string(), semesterId: z.string() }), req);
  ...
  for (const { studentId } of enrollments) {
    ...
    if (!gpa) { skipped.push(studentId); continue; } // approve grades first
  ...
  await logAudit(req, 'GENERATE_REPORT_CARDS', 'ReportCard', undefined, { classId, semesterId, generated });
  res.json({ generated, skipped: skipped.length });
}));

// GET /api/report-cards?classId&semesterId
reportCardsRouter.get('/', authenticate, authorize(Role.ADMIN, Role.TEACHER), ah(async (req, res) => {
  const { classId, semesterId } = parseQuery(z.object({ classId: z.string(), semesterId: z.string() }), req);
  const enrollments = await prisma.enrollment.findMany({ where: { classId, semesterId }, select: { studentId: true } });
  ...
  res.json({
    data: cards.map((c) => ({
      ...
      student: {
        ...
        className: c.student.classRoom ? `${c.student.classRoom.name} ${c.student.classRoom.stream}` : '—',
      },
...
reportCardsRouter.get('/mine', authenticate, authorize(Role.STUDENT, Role.PARENT), ah(async (req, res) => {
  const me = req.user!;
  let studentId: string | undefined;

  if (me.role === Role.STUDENT) {
    const profile = await prisma.studentProfile.findFirst({ where: { userId: me.id }, select: { id: true } });
...
function serializeSignatures(data: FullCard) {
  const sig = (s: { name: string; title: string; png: Buffer | null } | null) =>
    s === null ? null : { name: s.name, title: s.title, dataUrl: s.png ? `data:image/png;base64,${s.png.toString('base64')}` : null };
  return { ...data, signatures: { classTeacher: sig(data.signatures?.classTeacher ?? null), principal: sig(data.signatures?.principal ?? null) } };
}

// GET /api/report-cards/verify/:code — PUBLIC verification used by the QR code
reportCardsRouter.get('/verify/:code', ah(async (req, res) => {
  const card = await prisma.reportCard.findUnique({ where: { verificationCode: req.params.code } });
...
function canAccessCard(user: Express.Request['user'], cardStudent: { userId: string; parent: { userId: string } | null }, status: string) {
...
reportCardsRouter.get('/:id/pdf', authenticate, ah(async (req, res) => {
  const card = await prisma.reportCard.findUnique({
    where: { id: req.params.id },
    include: { student: { select: { userId: true, parent: { select: { userId: true } } } } },
  });
...
reportCardsRouter.patch('/:id/remarks', authenticate, authorize(Role.ADMIN), ah(async (req, res) => {
...
async function publishCards(cardIds: string[], adminId: string) {
  ...
  const audience = cards.flatMap((c) => [c.student.userId, c.student.parent?.userId].filter(Boolean) as string[]);
  await notifyUsers(audience, 'REPORT_CARD_AVAILABLE', 'Report card available',
    'Your report card for this term is now available. Open the Report Cards page to view or download it.', '/report-cards');
  return audience.length;
}

reportCardsRouter.post('/:id/publish', authenticate, authorize(Role.ADMIN), ah(async (req, res) => {
...
reportCardsRouter.post('/publish-all', authenticate, authorize(Role.ADMIN), ah(async (req, res) => {
  const { classId, semesterId } = parseBody(z.object({ classId: z.string(), semesterId: z.string() }), req);
  const enrollments = await prisma.enrollment.findMany({ where: { classId, semesterId }, select: { studentId: true } });
  const cards = await prisma.reportCard.findMany({
    where: { semesterId, studentId: { in: enrollments.map((e) => e.studentId) }, status: 'GENERATED' },
    select: { id: true },
  });
  const notified = await publishCards(cards.map((c) => c.id), req.user!.id);
