  ...
  ah(async (req, res) => {
    ...
    if (student.parentId && student.parentId !== parent.id) {
      throw AppError.conflict(
        `“${student.user.name}” is already linked to another parent. Unlink them there or edit the student record.`,
      );
    }
...
parentsRouter.delete(
  ...
  ah(async (req, res) => {
    ...
    await logAudit(req, 'UNLINK_PARENT_CHILD', 'StudentProfile', student.id, {
      parentId: parent.id,
    });
    res.json({ success: true });
  }),
...
parentsRouter.delete(
  ...
  ah(async (req, res) => {
    ...
    await logAudit(req, 'DELETE_PARENT', 'ParentProfile', existing.id, {
      email: existing.user.email,
    });
    res.json({ success: true });
  }),
