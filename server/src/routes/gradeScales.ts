...
gradeScalesRouter.get('/', ah(async (_req, res) => {
...
function validateBands(bands: z.infer<typeof bandSchema>[]) {
  ...
  if (letters.size !== bands.length) throw AppError.badRequest('Duplicate grade letters are not allowed');
  const mins = bands.map((b) => b.minScore).sort((a, b) => a - b);
  for (let i = 1; i < mins.length; i++) {
    if (mins[i] === mins[i - 1]) throw AppError.badRequest('Overlapping grade bands are not allowed');
  }
  for (const b of bands) {
    if (b.maxScore < b.minScore) throw AppError.badRequest(`Band ${b.letter}: maxScore must be ≥ minScore`);
  }
}

gradeScalesRouter.post('/', authorize(Role.ADMIN), ah(async (req, res) => {
  const body = parseBody(z.object({ name: z.string().min(2), bands: z.array(bandSchema).min(2) }), req);
...
gradeScalesRouter.put('/:id', authorize(Role.ADMIN), ah(async (req, res) => {
  const body = parseBody(z.object({ name: z.string().min(2).optional(), bands: z.array(bandSchema).min(2) }), req);
...
gradeScalesRouter.post('/:id/activate', authorize(Role.ADMIN), ah(async (req, res) => {
  const scale = await prisma.gradeScale.findUnique({ where: { id: req.params.id }, include: { bands: true } });
  if (!scale) throw AppError.notFound('Grade scale');
  if (scale.bands.length === 0) throw AppError.badRequest('Cannot activate a scale with no bands');
...
gradeScalesRouter.delete('/:id', authorize(Role.ADMIN), ah(async (req, res) => {
  const scale = await prisma.gradeScale.findUnique({ where: { id: req.params.id } });
  if (!scale) throw AppError.notFound('Grade scale');
