...
studentsRouter.get(
  ...
  ah(async (req, res) => {
    ...
    const orderBy =
      query.sortBy === 'admissionNumber'
        ? { admissionNumber: sortDirection(query.sortDir) }
        : query.sortBy === 'name'
          ? { user: { name: sortDirection(query.sortDir) } }
...
studentsRouter.post(
  ...
  ah(async (req, res) => {
    ...
    const student = await withIdRetry(() =>
      prisma.$transaction(async (tx) => {
        const admissionNumber =
          body.admissionNumber ?? (await generateAdmissionNumber(tx, school.studentIdPrefix, year));
        ...
        if (body.classId) {
          try {
            ...
            /* no active semester yet — harmless */
          }
        }
        return profile;
      }),
    ...
    await logAudit(req, 'CREATE_STUDENT', 'StudentProfile', student.id, {
...
function mapRow(raw: Record<string, string>): Record<(typeof IMPORT_HEADERS)[number], string> {
  ...
  for (const key of IMPORT_HEADERS) {
    out[key] = (
      HEADER_ALIASES[key].map((a) => byNormalized.get(a)).find((v) => v !== undefined) ?? ''
    ).trim();
  }
  return out;
}
...
studentsRouter.get(
  ...
  ah(async (_req, res) => {
    ...
    ws.columns = [
      ...
      { header: 'dateOfBirth', width: 14 },
      { header: 'gender', width: 10 },
      { header: 'class', width: 14 },
      { header: 'stream', width: 10 },
      { header: 'parentEmail', width: 28 },
...
const importRowSchema = z.object({
  ...
  dateOfBirth: z
    .string()
    .refine((s) => s !== '' && !Number.isNaN(Date.parse(s)), 'dateOfBirth must be YYYY-MM-DD'),
...
function resolveClassId(
  row: { class: string; stream: string },
  lookup: ClassLookup,
): string | null {
  if (!row.class) return null;
  if (row.stream)
    return lookup.byPair.get(`${lookup.norm(row.class)}|${lookup.norm(row.stream)}`) ?? null;
...
async function validateImportRows(rawRows: Record<string, string>[]): Promise<{
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (row.password) {
      ...
      if (!pwCheck.success) {
        fail(
          'Password must be at least 8 characters and include both a letter and a number. Leave blank to auto-generate.',
        );
        continue;
      }
    }
    if (seenEmails.has(row.email)) {
      fail(
        `Duplicate email “${row.email}” appears earlier in this file. Each student needs a unique email.`,
      );
      continue;
    }
    ...
    if (row.parentEmail && !parentByEmail.has(row.parentEmail)) {
      fail(
        `No parent account exists for “${row.parentEmail}”. Create the parent first, or leave parentEmail blank.`,
      );
      continue;
    }
    ...
    valid.push({
      ...
      classLabel: row.class
        ? `${row.class}${row.stream ? ` ${row.stream}` : ''}`
        : '— Unassigned —',
      classId,
  ...
  ah(async (req, res) => {
    ...
    if (rawRows.length === 0)
      throw AppError.badRequest(
        'The file contains no data rows below the header. Add students starting at row 2.',
      );
    if (rawRows.length > 500)
      throw AppError.badRequest(
        'Too many rows — split the file into batches of 500 students or fewer.',
      );
...
studentsRouter.post(
  ...
  ah(async (req, res) => {
    ...
    try {
      activeSemesterId = (await getActiveSemester()).id;
    } catch {
      /* no active term — skip enrolment */
    }
      ...
      try {
        await withIdRetry(() =>
          prisma.$transaction(async (tx) => {
            ...
            if (row.classId && activeSemesterId) {
              await tx.enrollment.upsert({
                where: {
                  studentId_semesterId: { studentId: profile.id, semesterId: activeSemesterId },
                },
                create: {
        ...
        if ((err as { code?: string })?.code === 'P2002') {
          ...
          result.errors.push({
            row: row.row,
            email: row.email,
            reason: `Email “${row.email}” is already registered. Skip this row or use a different email.`,
          });
        } else throw err;
      }
    }
...
studentsRouter.delete(
  ...
  ah(async (req, res) => {
    ...
    await logAudit(req, 'DELETE_STUDENT', 'StudentProfile', existing.id, {
      admissionNumber: existing.admissionNumber,
    });
...
studentsRouter.get(
  ...
  ah(async (req, res) => {
    ...
    const { semesterId: qsSemester, all } = parseQuery(
      z.object({
