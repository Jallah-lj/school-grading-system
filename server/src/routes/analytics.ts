...
analyticsRouter.get('/dashboard', authorize(Role.ADMIN, Role.TEACHER), ah(async (_req, res) => {
  ...
  if (activeSemester) {
    ...
    distribution = dist.map((d) => ({ letter: d.letterGrade, count: d._count.letterGrade }))
    ...
    pendingSubmissions = new Set(submittedEntries.map((e) => `${e.subjectId}:${e.student.classId}`)).size;

    const studentCard = {
      student: {
        include: { user: { select: { name: true } }, classRoom: { select: { name: true, stream: true } } },
      },
    } as const;
    ...
    const [top, bottom] = await Promise.all([
      prisma.gPARecord.findMany({
        where: { semesterId: activeSemester.id, isPublished: true },
        orderBy: { gpa: 'desc' }, take: 5, include: studentCard,
      }),
      prisma.gPARecord.findMany({
        where: { semesterId: activeSemester.id, isPublished: true },
        orderBy: { gpa: 'asc' }, take: 5, include: studentCard,
      }),
    ]);
    const shape = (r: (typeof top)[number]) => ({
      studentId: r.studentId,
      name: r.student.user.name,
      className: r.student.classRoom ? `${r.student.classRoom.name} ${r.student.classRoom.stream}` : '—',
      gpa: r.gpa,
      average: r.average,
      position: r.position,
    });
    ...
    recentResults = (await prisma.subjectResult.findMany({
      where: { semesterId: activeSemester.id, isPublished: true },
  ...
  const gpaTrend = await Promise.all(semesters.map(async (s) => {
    const agg = await prisma.gPARecord.aggregate({
      where: { semesterId: s.id, isPublished: true },
      _avg: { gpa: true },
    });
    return { semester: s.name, year: s.academicYear.name, average: agg._avg.gpa === null ? null : round2(agg._avg.gpa) };
  }));
...
analyticsRouter.get('/subject-performance', authorize(Role.ADMIN, Role.TEACHER), ah(async (req, res) => {
  ...
  for (const r of results) {
    if (!bySubject.has(r.subjectId)) bySubject.set(r.subjectId, { code: r.subject.code, name: r.subject.name, values: [] });
    bySubject.get(r.subjectId)!.values.push(r.percentage);
  }

  const data = [...bySubject.entries()].map(([subjectId, s]) => ({
    subjectId,
    code: s.code,
    name: s.name,
...
analyticsRouter.get('/gpa-trends', ah(async (req, res) => {
...
analyticsRouter.get('/class-performance', authorize(Role.ADMIN, Role.TEACHER), ah(async (req, res) => {
  ...
  const data = await Promise.all(classIds.map(async ([classId, classRoom]) => {
