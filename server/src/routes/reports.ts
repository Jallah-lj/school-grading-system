...
reportsRouter.get('/gradesheet.csv', authorize(Role.TEACHER, Role.ADMIN), ah(async (req, res) => {
  const { classId, subjectId, semesterId } = parseQuery(
    z.object({ classId: z.string(), subjectId: z.string(), semesterId: z.string() }), req);

  if (req.user!.role !== Role.ADMIN) {
    const assigned = await prisma.teacherAssignment.findFirst({
      where: { classId, subjectId, teacher: { userId: req.user!.id } },
    });
    if (!assigned) throw AppError.forbidden('You are not assigned to this subject for this class');
  }
  ...
  const [components, entries, results, subject, classRoom, semester] = await Promise.all([
    ...
    prisma.subjectResult.findMany({ where: { subjectId, semesterId, studentId: { in: studentIds } } }),
  ...
  const header = ['Admission No', 'Student', ...components.map((c) => `${c.name} (/${c.maxScore})`), 'Total %', 'Grade', 'Rank', 'Remark'];
  ...
  const filename = `gradesheet_${subject?.code}_${classRoom?.name}_${semester?.name}`.replace(/\s+/g, '_') + '.csv';
  const meta = [
...
reportsRouter.get('/class-report.csv', authorize(Role.TEACHER, Role.ADMIN), ah(async (req, res) => {
  ...
  const header = ['Admission No', 'Student', ...subjects.map((s) => s.code), 'Average', 'GPA', 'Position'];
  const filename = `class_report_${classRoom?.name}_${semester?.name}`.replace(/\s+/g, '_') + '.csv';
  const meta = [
