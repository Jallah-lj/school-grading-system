...
import { ensureEnrollments, recomputeGpas, recomputeSubjectResults } from '../services/results.service';
...
const scopeSchema = z.object({ classId: z.string(), subjectId: z.string(), semesterId: z.string() });

/** Teachers may only touch (class, subject) pairs they are assigned to. Admins bypass. */
async function assertTeacherScope(user: Express.Request['user'], classId: string, subjectId: string) {
...
async function buildGrid(classId: string, subjectId: string, semesterId: string, isAdmin: boolean) {
  ...
  return {
    ...
    components: components.map((c) => ({
      id: c.id, type: c.type, name: c.name, weight: c.weight, maxScore: c.maxScore,
    })),
    entries: gridEntries,
    status: lowest ?? 'EMPTY',
    editable: isAdmin ? lowest !== 'PUBLISHED' : lowest === null || lowest === 'DRAFT' || lowest === 'SUBMITTED',
  };
}

// GET /api/grades/grid?classId&subjectId&semesterId — the teacher's mark-entry matrix
gradesRouter.get('/grid', authorize(Role.TEACHER, Role.ADMIN), ah(async (req, res) => {
...
gradesRouter.post('/entry', authorize(Role.TEACHER, Role.ADMIN), ah(async (req, res) => {
  ...
  const components = await prisma.assessmentComponent.findMany({ where: { subjectId: body.subjectId } });
  ...
  await logAudit(req, 'ENTER_GRADES', 'GradeEntry', undefined, {
    classId: body.classId, subjectId: body.subjectId, semesterId: body.semesterId, students: body.entries.length,
  });
  res.json(await buildGrid(body.classId, body.subjectId, body.semesterId, req.user!.role === Role.ADMIN));
}));

// POST /api/grades/submit — teacher submits DRAFT marks for approval
gradesRouter.post('/submit', authorize(Role.TEACHER, Role.ADMIN), ah(async (req, res) => {
  const body = parseBody(scopeSchema, req);
  await assertTeacherScope(req.user, body.classId, body.subjectId);

  const result = await prisma.gradeEntry.updateMany({
    where: {
      subjectId: body.subjectId, semesterId: body.semesterId,
      student: { classId: body.classId }, status: 'DRAFT',
    },
    data: { status: 'SUBMITTED', submittedAt: new Date() },
  });

  const [subject, classRoom] = await Promise.all([
    prisma.subject.findUnique({ where: { id: body.subjectId }, select: { name: true, code: true } }),
    prisma.classRoom.findUnique({ where: { id: body.classId }, select: { name: true, stream: true } }),
  ]);
  const label = `${classRoom?.name ?? ''} ${classRoom?.stream ?? ''} — ${subject?.name ?? ''} (${subject?.code ?? ''})`.replace(/\s+/g, ' ').trim();

  const admins = await prisma.user.findMany({ where: { role: 'ADMIN', isActive: true }, select: { id: true } });
  await notifyUsers(
    admins.map((a) => a.id), 'ANNOUNCEMENT', 'Grades awaiting approval',
  ...
  res.json({ submitted: result.count, grid: await buildGrid(body.classId, body.subjectId, body.semesterId, req.user!.role === Role.ADMIN) });
}));
...
gradesRouter.get('/pending-approvals', authorize(Role.ADMIN), ah(async (_req, res) => {
  const entries = await prisma.gradeEntry.findMany({
    where: { status: 'SUBMITTED' },
    select: {
      studentId: true, subjectId: true, semesterId: true, submittedAt: true,
  ...
  interface Group {
    ...
    marks: number; students: Set<string>; submittedAt: Date | null;
  }
  ...
  for (const e of entries) {
    ...
    if (!g) {
      g = { classId, subjectId: e.subjectId, semesterId: e.semesterId, marks: 0, students: new Set(), submittedAt: null };
      groups.set(key, g);
    }
    g.marks += 1;
    g.students.add(e.studentId);
    if (e.submittedAt && (!g.submittedAt || e.submittedAt < g.submittedAt)) g.submittedAt = e.submittedAt;
  }
  ...
  const [classes, subjects, semesters, assignments] = await Promise.all([
    prisma.classRoom.findMany({ where: { id: { in: classIds } }, select: { id: true, name: true, stream: true } }),
    prisma.subject.findMany({ where: { id: { in: subjectIds } }, select: { id: true, name: true, code: true } }),
    prisma.semester.findMany({ where: { id: { in: semesterIds } }, include: { academicYear: { select: { name: true } } } }),
  ...
  const data = rows.map((g) => {
    ...
    return {
      classId: g.classId, className: cls?.name ?? '—', stream: cls?.stream ?? '',
      subjectId: g.subjectId, subjectName: subject?.name ?? '—', subjectCode: subject?.code ?? '',
      semesterId: g.semesterId, semesterName: semester?.name ?? '—',
      academicYearName: semester?.academicYear.name ?? '',
      marks: g.marks, students: g.students.size,
...
gradesRouter.post('/approve', authorize(Role.ADMIN), ah(async (req, res) => {
  const body = parseBody(scopeSchema, req);

  const submitted = await prisma.gradeEntry.count({
    where: { subjectId: body.subjectId, semesterId: body.semesterId, student: { classId: body.classId }, status: 'SUBMITTED' },
  });
  ...
  const { computed } = await recomputeSubjectResults(body.classId, body.subjectId, body.semesterId);
  await recomputeGpas(body.classId, body.semesterId);
...
gradesRouter.post('/publish', authorize(Role.ADMIN), ah(async (req, res) => {
  const body = parseBody(scopeSchema, req);
  ...
  const approved = await prisma.gradeEntry.count({
    where: { subjectId: body.subjectId, semesterId: body.semesterId, studentId: { in: studentIds }, status: 'APPROVED' },
  });
  ...
  await notifyUsers(audience, 'GRADES_PUBLISHED', 'Grades published',
    `Results for ${subject} have been published. Check your report card for details.`, '/grades');

  await logAudit(req, 'PUBLISH_GRADES', 'GradeEntry', undefined, { ...body, published: approved });
...
gradesRouter.post('/unlock', authorize(Role.ADMIN), ah(async (req, res) => {
  const body = parseBody(scopeSchema.extend({
  ...
  const updated = await prisma.gradeEntry.updateMany({
    where: { subjectId: body.subjectId, semesterId: body.semesterId, studentId: { in: studentIds }, status: from },
  ...
  await notifyUsers(
    teachers.map((t) => t.teacher.user.id), 'GRADE_CORRECTION',
  ...
  await logAudit(req, returning ? 'RETURN_GRADES' : 'UNLOCK_GRADES', 'GradeEntry', undefined, { ...body });
  res.json({ unlocked: updated.count });
}));

// GET /api/grades/class-summary?classId&semesterId — class grade sheet
gradesRouter.get('/class-summary', authorize(Role.TEACHER, Role.ADMIN), ah(async (req, res) => {  const { classId, semesterId } = parseQuery(z.object({ classId: z.string(), semesterId: z.string() }), req);
  const enrollments = await prisma.enrollment.findMany({
    where: { classId, semesterId },
    include: { student: { include: { user: { select: { name: true } } } } },
    orderBy: { student: { user: { name: 'asc' } } },
  ...
  const rows = await Promise.all(enrollments.map(async (en) => {
    const [results, gpa] = await Promise.all([
      ...
      prisma.gPARecord.findUnique({ where: { studentId_semesterId: { studentId: en.studentId, semesterId } } }),
    ]);
    return {
...
async function loadImportContext(classId: string, subjectId: string, semesterId: string) {
  ...
  const [classRoom, subject, semester, components, enrollments, entries] = await Promise.all([
    prisma.classRoom.findUnique({ where: { id: classId }, select: { id: true, name: true, stream: true } }),
    prisma.subject.findUnique({ where: { id: subjectId }, select: { id: true, name: true, code: true } }),
    prisma.semester.findUnique({ where: { id: semesterId }, include: { academicYear: { select: { name: true } } } }),
    prisma.assessmentComponent.findMany({ where: { subjectId } }),
    prisma.enrollment.findMany({
      where: { classId, semesterId },
      include: { student: { include: { user: { select: { name: true } } } } },
 ...
 */
gradesRouter.get('/import/template', authorize(Role.TEACHER, Role.ADMIN), ah(async (req, res) => {
  ...
  const scoreByKey = new Map(ctx.entries.map((e) => [`${e.studentId}:${e.componentId}`, e.score]));
  ...
  const fname = `marks_${ctx.classRoom.name}_${ctx.classRoom.stream}_${ctx.subject.code}.xlsx`.replace(/\s+/g, '_');
 ...
 */
gradesRouter.post('/import', authorize(Role.TEACHER, Role.ADMIN), spreadsheetUpload.single('file'), ah(async (req, res) => {
  const { classId, subjectId, semesterId } = parseQuery(importScopeQuery, req);
  await assertTeacherScope(req.user, classId, subjectId);
  if (!req.file) throw AppError.badRequest('Attach an .xlsx or .csv file (max 5 MB)');
  const ctx = await loadImportContext(classId, subjectId, semesterId);
  if (ctx.components.length === 0) throw AppError.badRequest('This subject has no assessment components configured');
  if (ctx.enrollments.length === 0) throw AppError.badRequest('No students are enrolled in this class for the term');

  const rows = await parseSpreadsheetFile(req.file);
  if (rows.length === 0) throw AppError.badRequest('The file contains no data rows below the header');
  ...
  const compByNormName = new Map(ctx.components.map((c) => [c.name.toLowerCase().replace(/\s+/g, ' ').trim(), c]));
  const headers = Object.keys(rows[0]);
  const admHeader = headers.find((h) => ADMISSION_HEADERS.includes(normalizeCol(h)));
  if (!admHeader) throw AppError.badRequest('An "admissionNumber" column is required — use the downloadable template');
  ...
  if (compCols.size === 0) throw AppError.badRequest('No component columns found — use the downloadable template');

  const byAdmission = new Map(ctx.enrollments.map((e) => [e.student.admissionNumber.toLowerCase(), e.studentId]));
  const statusByKey = new Map(ctx.entries.map((e) => [`${e.studentId}:${e.componentId}`, e.status]));

  const errors: { row: number; admissionNumber: string; component: string; reason: string }[] = [];
  interface Op { studentId: string; componentId: string; score: number }
  const ops: Op[] = [];
  let skipped = 0;
  ...
  rows.forEach((row, i) => {
    ...
    if (!admRaw) {
      errors.push({ row: rowNo, admissionNumber: '—', component: '—', reason: 'missing admission number' });
      return;
    }
    const studentId = byAdmission.get(admRaw.toLowerCase());
    if (!studentId) {
      errors.push({ row: rowNo, admissionNumber: admRaw, component: '—', reason: 'no such student in this class' });
      return;
    }
    for (const [header, comp] of compCols) {
      ...
      if (!Number.isFinite(score)) {
        errors.push({ row: rowNo, admissionNumber: admRaw, component: comp.name, reason: `"${raw}" is not a number` });
        continue;
      }
      if (score < 0 || score > comp.maxScore) {
        errors.push({ row: rowNo, admissionNumber: admRaw, component: comp.name, reason: `out of range 0–${comp.maxScore}` });
        continue;
      }
      ...
      else { opIndex.set(mapKey, ops.length); ops.push({ studentId, componentId: comp.id, score }); }
    }
  });
  ...
  for (const op of ops) {
    ...
    if (st === 'PUBLISHED') throw AppError.conflict('Marks are published and locked. Ask an administrator to unlock them first.');
    if (st === 'APPROVED' && !isAdmin) throw AppError.conflict('Marks have been approved and can no longer be edited by teachers. Ask an administrator to unlock them.');
  }
  ...
  await logAudit(req, 'IMPORT_GRADES', 'GradeEntry', undefined, {
    classId, subjectId, semesterId, applied: ops.length, skipped, failed: errors.length, file: req.file.originalname,
  });
  res.json({ applied: ops.length, skipped, failed: errors.length, errors, file: req.file.originalname, recomputed: overwroteApproved });
}));
