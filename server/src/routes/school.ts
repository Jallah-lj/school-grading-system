  ...
  ah(async (req, res) => {
    const body = parseBody(
      z.object({
        ...
        studentIdPrefix: z
          .string()
          .regex(/^[A-Z0-9]{2,6}$/, '2–6 uppercase letters/digits')
          .optional(),
      }),
      req,
    );
    ...
    await logAudit(req, 'UPDATE_SCHOOL_SETTINGS', 'SchoolSetting', 'school', {
      name: s.name,
      studentIdPrefix: s.studentIdPrefix,
    });
    res.json({
...
schoolRouter.post(
  ...
  ah(async (req, res) => {
    ...
    await logAudit(req, 'UPDATE_SCHOOL_BADGE', 'SchoolSetting', 'school', {
