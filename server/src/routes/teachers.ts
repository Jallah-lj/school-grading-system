...
teachersRouter.post(
  ...
  ah(async (req, res) => {
    ...
    const teacher = await withIdRetry(() =>
      prisma.$transaction(async (tx) => {
        const staffNumber =
          body.staffNumber ?? (await generateStaffNumber(tx, school.studentIdPrefix));
    ...
    await logAudit(req, 'CREATE_TEACHER', 'TeacherProfile', teacher.id, {
...
teachersRouter.delete(
  ...
  ah(async (req, res) => {
    ...
    await logAudit(req, 'DELETE_TEACHER', 'TeacherProfile', existing.id, {
      staffNumber: existing.staffNumber,
    });
    res.json({ success: true });
  }),
