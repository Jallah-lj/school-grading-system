 ...
 */
signaturesRouter.post('/me', upload.single('file'), ah(async (req, res) => {
  if (!req.file) {
    throw AppError.badRequest('Attach an image file in the `file` field (PNG, JPG or WebP, max 5 MB)');
  }

  const defaultTitle = req.user!.role === Role.ADMIN ? 'Principal / Head of School' : 'Class Teacher';
  ...
  if (processed.png.length > 350_000) {
    throw AppError.badRequest('Signature image is still too large after compression — retake a closer photo with less background');
  }

  const signature = await prisma.signature.upsert({
    where: { userId: req.user!.id },
    create: { userId: req.user!.id, title, data: processed.png, width: processed.width, height: processed.height },
  ...
  await logAudit(req, 'UPLOAD_SIGNATURE', 'Signature', signature.id, {
    bytes: processed.png.length, width: processed.width, height: processed.height,
  });
  res.status(201).json(signature);
}));

// GET /api/signatures/me/meta — does the current user have a signature saved?
signaturesRouter.get('/me/meta', ah(async (req, res) => {
...
signaturesRouter.get('/me', ah(async (req, res) => {
...
signaturesRouter.get('/user/:userId', authorize(Role.ADMIN), ah(async (req, res) => {
...
signaturesRouter.delete('/me', ah(async (req, res) => {
